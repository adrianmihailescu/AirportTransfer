    ...
    public partial class ForeignKeyFilter : System.Web.DynamicData.QueryableFilterUserControl
    {
        ...
        protected void Page_Init(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                lnkEntityNew.ToolTip = ConfigurationManager.AppSettings["ForeignKeyTextNew"];
                ...
                lnkEntityDetails.ToolTip = ConfigurationManager.AppSettings["ForeignKeyTextDetails"]; ;
        ...
        public override IQueryable GetQueryable(IQueryable source)
        {
            ...
            string queryStringColumn="", queryStringValue="", queryStringNumber="";
            
            if (!String.IsNullOrEmpty(queryString))
            {
                queryStringValue = queryString.Substring(queryString.IndexOf("ID") + 2);
                queryStringNumber = queryStringValue.Substring(queryStringValue.IndexOf('=') + 1);
                queryStringColumn = queryStringValue.Substring(0, queryStringValue.IndexOf("="));
            }

            if (queryStringColumn.ToLower().Equals(Column.Name.ToLower()))
            {
                if (!Utils.IsIdInListValues(queryStringNumber, ListBox1))
                {
